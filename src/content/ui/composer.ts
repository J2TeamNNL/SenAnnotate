...
export interface ComposerCallbacks {
  onSubmit(comment: string, kind: AnnotationKind, design: ComposerDesign, referenceImages: string[]): void;
  onCancel(): void;
  onScreenshot(): void;
...
export class Composer {
   ...
   * changes height after that decision was taken — attaching an image grows it, opening
   * the Design section grows it, and a retarget rebuilds a taller meta block. All three
   * have to re-clamp against the anchor the first decision was made against, not against
   * a `top` frozen when the card was shorter.
   */
  ...
  constructor(
    ...
    this.element = h(
      ...
      h("div", { class: "card__body" }, this.meta, kinds, this.textarea, this.design?.element ?? null, this.strip, this.fileInput),
      footer,
    );
