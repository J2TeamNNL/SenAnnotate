...
So the rule is local and absolute: **every interpolation goes through `esc()`**, with no
exception for fields that look safe. An element name is `button "Save"` today and whatever a
page author wrote tomorrow. There is one e2e check (`an element name cannot close a tag in
the shared file`) whose entire job is to fail if someone adds a `${}` without it.
